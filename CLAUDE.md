...
The app uses expo-sqlite with the following key tables:

**realms** → **containers** → **items** ← **tags**

### Tags Table (Schema v3)
- Items can have multiple tags (many-to-many relationship)
- Tags are stored as separate rows in the `tags` table
- Each tag row references an `item_id` and has a `tag_name`
- Common tags: "FOR TRADE", custom user tags
- Tag filtering in Find Item uses INNER JOIN to filter items by tag

### Common Queries
All database query functions live in `db/queries.ts`:
- `searchItemsAcrossContainers()` — Find Item search with realm/category/tag filtering
- `getItemsByContainer()` — Container detail view items list
- `addTagToItem()`, `removeTagFromItem()` — Tag management
- `getTagsForItem()` — Get all tags for a specific item
...
- [ ] Enhanced runeword calculator — filter by base type, show socket requirements
...
7. **Tag filtering requires INNER JOIN.** When filtering by tags in Find Item, the query MUST use `INNER JOIN tags ON tags.item_id = items.id` plus a WHERE clause filtering `tags.tag_name`. LEFT JOIN will return all items (with nulls for untagged), which breaks the filter. Also watch for case sensitivity in tag_name matching.
